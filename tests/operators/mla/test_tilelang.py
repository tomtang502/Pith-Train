"""
Test the correctness of the MLA operator.

Run all tests:
    pytest tests/operators/mla/test_tilelang.py -v
Run only backward tests:
    pytest tests/operators/mla/test_tilelang.py -v -k "bwd"
"""
...
from pithtrain.operators.mla.tilelang import MLA as MLATileLang
...
def workload(request: pytest.FixtureRequest) -> Workload:
    ...
    MLATileLang.autotune(b, s, h, dq, dv, softmax_scale)
...
def test_fwd(workload: Workload) -> None:
    ...
    our = MLATileLang(h, dq, dv, softmax_scale).cuda()
